...
export default function VolunteerListView({ volunteers, currentRole }: VolunteerListViewProps) {
    const canSeeAllPhones = currentRole === OrganizationRole.ADMIN || currentRole === OrganizationRole.TREASURER;

    const hasPhoneAccess = (volunteer: Volunteer) => {
        if (canSeeAllPhones) return true;
        if (currentRole === OrganizationRole.COMMITTEE_MEMBER) {
            return volunteer.role === OrganizationRole.COMMITTEE_MEMBER || volunteer.role === OrganizationRole.VOLUNTEER;
        }
        return false;
    };
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {volunteers.map((volunteer) => {
                    const visiblePhone = hasPhoneAccess(volunteer) ? volunteer.phone : null;
                    const isPhoneHidden = !hasPhoneAccess(volunteer) && volunteer.phone;
                        ...
                        >
                            ...
                            <div>
                                ...
                                <div className="space-y-0.5 mt-1">
                                    ...
                                    {visiblePhone ? (
                                        <p className="text-[10px] text-indigo-600 font-black flex items-center gap-1">
                                            <Phone className="w-2.5 h-2.5" />
                                            {visiblePhone}
                                        </p>
                                    ) : isPhoneHidden ? (
                                        <p className="text-[9px] text-slate-300 font-black italic uppercase tracking-tighter">
                                            [Phone Restricted]
                                        </p>
                                    ) : null}
                                </div>
